...
public class HebWriter
{
    public byte[] Write(HebHeader heb)
    {
...
public class HebReader : IDisposable
{
    private readonly FileStream _stream;

    public HebReader(string path)
    {
        _stream = new FileStream(path, FileMode.Open, FileAccess.Read);
    }

    public HebHeader Read()
    {
        using var reader = new BinaryReader(_stream, Encoding.UTF8);
        ...
        for (var i = 0; i < header.ImageCount; i++)
        {
            var imageHeader = new HebImageHeader
            {
                ...
                TextureDataOffsetOffset = _stream.Position,
            ...
            var returnAddress = _stream.Position;
            _stream.Seek(imageHeader.TextureDataOffsetOffset + imageHeader.TextureDataOffset, SeekOrigin.Begin);
            reader.Read(imageHeader.DdsData);
            _stream.Seek(returnAddress, SeekOrigin.Begin);
