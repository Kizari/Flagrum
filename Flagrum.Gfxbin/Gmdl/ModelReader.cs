        ...
        private void ReadMeshData()
        {
            ...
            for (var _ = 0; _ < meshObjectCount; _++)
            {
                ...
                foreach (var x in Enumerable.Range(0, (int)meshCount))
                {
                    ...
                    mesh.IndexCount = _reader.ReadUint();
                    mesh.IndexType = (IndexType)_reader.ReadByte();
                    mesh.IndexBufferOffset = _reader.ReadUint();
                    mesh.IndexBufferSize = _reader.ReadUint();
                    mesh.IndexBuffer = new int[mesh.IndexCount];

                    foreach (var i in Enumerable.Range(0, (int)mesh.IndexCount))
                    {
                        switch (mesh.IndexType)
                        {
                            case IndexType.IndexType32:
                                mesh.IndexBuffer[i] =
                                    BitConverter.ToInt32(_gpuBuffer, (int)mesh.IndexBufferOffset + 4 * i);
                                break;
                            case IndexType.IndexType16:
                                mesh.IndexBuffer[i] =
                                    BitConverter.ToInt16(_gpuBuffer, (int)mesh.IndexBufferOffset + 2 * i);
                                break;
                            default:
                                throw new InvalidOperationException(
                                    $"Provided {nameof(mesh.IndexType)} not supported.");
                        }
                    }
