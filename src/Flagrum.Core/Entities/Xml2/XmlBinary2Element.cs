...
public class XmlBinary2Element
{
    ...
    private XmlBinary2Element(int elementStartOffset, int attributeStartOffset, uint elementCount, uint attributeCount,
    ...
    public void Dump(StringBuilder output, int indent)
    {
        ...
        for (var i = 0; i < AttributeCount; i++)
        {
            ...
            var variant = XmlBinary2Variant.FromByteArray(Xmb2, variantOffset);
    ...
    public static XmlBinary2Element FromByteArray(byte[] xmb2, long startIndex)
    {
        ...
        return new XmlBinary2Element(elementStartOffset, attributeStartOffset, (uint)elementCount, attributeCount, xmb2,
            startIndex);
    }
    ...
    public XmlBinary2Element GetElementByIndex(int index)
    {
        if (ElementCount == 0)
        {
            return null;
    ...
    /// <returns>The parsed child element.</returns>
    public XmlBinary2Element GetElementByName(string name)
    {
        if (ElementCount == 0)
        {
            return null;
    ...
    public XmlBinary2Variant GetAttributeByName(uint nameHash)
    {
        var variantRelativeOffsetTableOffset = Offset + AttributeStartOffset + 8;
        for (var i = 0; i < AttributeCount; i++)
        {
    ...
    public XmlBinary2Variant GetAttributeByName(string name)
    {
    ...
    public void GetElements(string name, IList<XmlBinary2Element> results)
    {
        for (var i = 0; i < ElementCount; i++)
        {
    ...
    public IEnumerable<XmlBinary2Element> GetElements(string name)
    {
        for (var i = 0; i < ElementCount; i++)
        {
            var element = GetElementByIndex(i);
    ...
    /// </summary>
    public IList<XmlBinary2Element> GetElements()
    {
        var result = new List<XmlBinary2Element>();
        for (var i = 0; i < ElementCount; i++)
        {
            var element = GetElementByIndex(i);
            if (element == null)
    ...
    private XmlBinary2Variant GetValueAttribute()
    {
        if (ElementCount != 0)
        {
            return null;
        ...
        return XmlBinary2Variant.FromByteArray(Xmb2, attributeOffset);
    }
    ...
    private string GetName()
    {
        ...
        var nameAttribute = XmlBinary2Variant.FromByteArray(Xmb2, nameAttributeOffset);
        ...
        if (nameAttribute.ValueType == XmlBinary2Variant.Type.String)
        {
            return nameAttribute.Value as string;
        }

        if (nameAttribute.ValueType != XmlBinary2Variant.Type.Bool)
        {
            return null;
        }
