...
[RegisterSingleton]
public partial class ModelReplacementPresets
{
    public ModelReplacementPresets()
    {
        // Load persisted model replacement presets
        Repository = Core.Persistence.Repository.Load<ModelReplacementRepository>(ModelReplacementRepository.Path)
                     ?? new ModelReplacementRepository();

        BuildDictionary();
    }

    public ModelReplacementRepository Repository { get; }

    private List<ModelReplacementTarget> Replacements { get; set; }

    public void Save()
    {
        Core.Persistence.Repository.Save(Repository, ModelReplacementRepository.Path);
    }
    ...
    {
        // Merge default presets with user-defined presets
        Replacements = GetDefaultReplacements();
        Replacements.AddRange(Repository.Presets.Select(e => new ModelReplacementTarget
        {
            Index = e.Id + 100,
            Name = e.Name,
            ModmetaName = $"Custom_{e.Id + 100}",
            Models = e.ReplacementPaths
        }));
    }
