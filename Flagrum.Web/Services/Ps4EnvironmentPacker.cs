...
public class Ps4EnvironmentPacker
{
    ...
    private readonly ConcurrentDictionary<string, Unpacker> _unpackers = new();

    public Ps4EnvironmentPacker(
        ILogger logger,
    ...
    public void Pack(string uri, string outputPath)
    {
        ...
        GetPathsRecursively(uri, GetFileByUri(outerContext, uri),
            null,
            null,
            1.0f,
            new List<float[]>());
        ...
        foreach (var (uri2, _) in _textures)
        {
            ...
            var btexData = GetFileByUri(context, uri2);
            ...
            try
            {
                ...
                btex.Bitmap.Save($@"{_texturesDirectory}\\{fileNameWithoutExtension}.tga", ImageFormat.Png);
            }
            catch (Exception e) { }
        }
    ...
    private void PackModel(string uri, string directory, int index)
    {
        ...
        var gfxbin = GetFileByUri(context, uri);
        var gpubinUri = uri.Replace(".gmdl", ".gpubin");
        var gpubin = GetFileByUri(context, gpubinUri);
        ...
        var meshData = new Gpubin
        {
            ...
            Meshes = model.MeshObjects.SelectMany(o => o.Meshes
                ...
                .Select(m =>
                {
                    ...
                    var materialData = GetFileByUri(innerContext, materialUri);
                    Material material;
                    try
                    {
    ...
    private IEnumerable<string> GetHighResTextures(Material material)
    {
        if (!string.IsNullOrWhiteSpace(material.HighTexturePackAsset))
        {
            ...
            for (var i = 0; i < 2; i++)
            {
                ...
                {
                    highTexturePack = GetFileByUri(context, highTexturePackUri);
                }
                else
                {
        ...
        float prefabScale, List<float[]> prefabRotations)
    {
        ...
        Parallel.For(0, elements.Count, counter =>
        {
            ...
            else if (typeAttribute == "SQEX.Ebony.Framework.Entity.EntityPackageReference")
            {
                ...
                var innerXmb2 = GetFileByUri(context, combinedUriString);
